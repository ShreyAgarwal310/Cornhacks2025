...
function buyResearch(type) {
  ...
  // Now the effective research cost increases as research levels increase.
  const effectiveResearchCost = researchItem.cost * Math.pow(RESEARCH_COST_MULTIPLIER, researchItem.count);

  if (money < effectiveResearchCost) return; // Not enough money
  ...
  const researchToFacilityMap = {
    ...
    // fusion research can be mapped if desired
  };
  ...
  if (facilityType) {
    setFacilities(prev => ({
      ...
      [facilityType]: {
        ...prev[facilityType],
        // Reduce the facility cost by 10% per research level
    ...
    <div>
      ...
      <div className="buy-sell-facilities">
          <div>
            ...
            {/* <DesignedButton clickFunction={buyFacility('coalPlant')} text={`Buy Coal Plant ${($
              {new Intl.NumberFormat("en-US").format(
                facilities.coalPlant.cost * facilities.coalPlant.multiplier
              )})}` /> */}
            <DesignedButton clickFunction={handleButtonClick()} text="Buy Factory" />
...
// <>
    ...
    // function buyResearch(type) {
  //   setResearch(prev => {
  //     const researchItem = prev[type];
  //     const cost = researchItem.cost * researchItem.multiplier;
  //     if (money >= cost) {
  //       setMoney(money - cost);
  //       const updatedResearchItem = {
  //         ...researchItem,
  //         count: researchItem.count + 1,
  //         multiplier: researchItem.multiplier + 0.1
  //       };
  ...
  //           ...prevFacilities,
  //           solarFarm: {
  //             ...prevFacilities.solarFarm,
  //             cost: prevFacilities.solarFarm.cost * 0.95
  //           }
  //         }));
  ...
  //           ...prevFacilities,
  //           windFarm: {
  //             ...prevFacilities.windFarm,
  //             cost: prevFacilities.windFarm.cost * 0.95
  //           }
  //         }));
  ...
  //           ...prevFacilities,
  //           nuclearPlant: {
  //             ...prevFacilities.nuclearPlant,
  //             cost: prevFacilities.nuclearPlant.cost * 0.95
  //           }
  //         }));
  //       }
